...
{
    private bool jump,run, isground,jumping,unjump;
    ...
    private void penduluming()
    {
        if (pendulum == true)
        {
            ...
            if (attack == true)
            {
                rigidbody.AddForce(direction * 10.0f, ForceMode.Acceleration);
    ...
    private void shootanker()
    {
        ...
        Vector3 direction = anker.transform.position - mousePos;
        anker.GetComponent<Rigidbody>().isKinematic = false;
        //anker.GetComponent<Rigidbody>().AddRelativeForce(direction.normalized * 50.0f, ForceMode.VelocityChange);
        anker.GetComponent<Rigidbody>().AddRelativeForce((Vector3.forward*2 + Vector3.up) * 100.0f, ForceMode.VelocityChange);
